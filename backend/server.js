...
app.post('/api/services/:id/beneficiaries', verifyToken, async (req, res) => {
  try {
    const serviceId = req.params.id;
    const { resident_id, fullname, address, birthdate, age, civil_status, work, monthly_income, is_voter, contact_no, notes } = req.body;
    ...
    const result = await query(
      `INSERT INTO service_beneficiaries (service_id, resident_id, fullname, address, birthdate, age, civil_status, work, monthly_income, is_voters, contact_no, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [serviceId, resident_id, fullname, address, birthdate, age, civil_status, work, monthly_income, is_voter, contact_no, notes]
    );
  ...
  try {
    ...
    if (request && request.username) {
      await sendMail(
        ...
        status === "approved"
          ? "Your request has been approved. Printing Barangay ID is only valid for 7 days."
